package com.zoom.bundle_imagecache.image3cache.image3chche;
...
import com.zoom.bundle_imagecache.image3cache.Utils;
...
public class ImageCache {
	...
	private ImageCache(Context context) {
		//返回的是java虚拟机（这个进程）能构从操纵系统那里挖到的最大的内存，以字节为单位
	 ...
	 */
	public void loadImages(ImageView imageView, String imageUrl, boolean priority) {
		try {
	 ...
	 * @param task
	 */
	private synchronized void addTaskToWaitQueue(ImageDownloadTask task,boolean priority) {
	 ...
	 *            优先排队权
	 */
	private synchronized void addTaskToQueue(ImageDownloadTask task, boolean priority) {
